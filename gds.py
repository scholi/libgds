...
# Copyright: $Copyright$
# Version: 1.0
# Last Revision: $Date: 2011-08-17 10:27:55 +0200 (mer 17 aoû 2011) $
# Modified by: $LastChangedBy: scholi $
# Last Modified: $LastChangedDate: 2011-08-17 10:27:55 +0200 (mer 17 aoû 2011) $
...
class GDSII:
	...
	def addMarker(self, pos, size=10000, width=1000, dose=1, loop=None):
		...
		self.addRect((pos[0]+(size-width)/2,pos[1],width,size),dose=dose,loop=loop)
		self.addRect((pos[0],pos[1]+(size-width)/2,size,width),dose=dose,loop=loop)
	...
	def uvRotate(self, alpha=0):
		alpha*=math.pi/180
		self.M=MatrMatrMul([[math.cos(alpha),math.sin(alpha)],[-math.sin(alpha),cos(alpha)]],self.M)
	def getType(self, t):
		if t in self.Type.values():
			return self.Type.keys()[self.Type.values().index(t)]
		else: return t
	...
	def addText(self, pos, txt, height=None, mag=22.22222, layer=0, width=0, dose=1,angle=0,loop=None):
